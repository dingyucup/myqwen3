...
class CachedPredictor:
    ...
    def generate_greedy(
        ...
        # 存储生成的token ids
        # generated_token_ids = torch.empty((batch_size, 0), dtype=torch.long).to(input_ids.device)
        ...
        for i in range(max_num_new_tokens):
            ...
            # 重复性惩罚（需要完整历史）
            all_input_ids = generated_token_ids if generated_token_ids.size(1) > prompt_length else input_ids
            ...
            if next_id == eos_id:
                ...
                generated_token_ids = torch.cat([generated_token_ids, next_id], dim=1)
                last_token_id = next_id

        return ["".join(tokenizer.decode(token_ids)) for token_ids in generated_token_ids.tolist()]
...
if __name__ == "__main__":
    ...
    input_ids = tokenizer_t.encode(text_t)
    input_ids_tensor = torch.tensor([input_ids]).to(device_t)
    ...
    print("[贪心采样]:", end="")
    result = CachedPredictor.generate_greedy(
    ...
    print("[top-k采样]", end=":")
    result = CachedPredictor.generate_top_k(
        input_ids=input_ids_tensor,
        model=model_t,
        tokenizer=tokenizer_t,
        max_num_new_tokens=128,
    ...
    print("[top-p采样]", end=":")
    result = CachedPredictor.generate_top_p(
        input_ids=input_ids_tensor,
        model=model_t,
        tokenizer=tokenizer_t,
        max_num_new_tokens=128,
    ...
    question_t  = "太阳系有多少行星？"
    answer_t = CachedPredictor.generate_answer(
