...
class Predictor:
    ...
    def generate_greedy(
        ...
        # 自回归生成
        model.eval()
        for _ in range(max_num_new_tokens):
            # 文本序列截断，超出后，扔掉前面的字符编号
            ids_cond = input_ids[:, -context_size:]

            with torch.no_grad():
                logits = model(ids_cond)  # 对整个token串中每个token作一个下一词的概率预测
        ...
        # 找到新生成的id
        # results = input_ids[:, -max_num_new_tokens:].tolist()

        results = input_ids.tolist()
        results = [tokenizer.decode(result) for result in results]
        return results
    ...
    def generate_answer(
        ...
        instruct_ids_tensor = torch.tensor([instruct_ids]).to(device)

        answer = {}
        if top_k is not None:
            outputs = cls.generate_top_k(
                input_ids=instruct_ids_tensor,
                model=model,
                tokenizer=tokenizer,
            ...
            answer["top_k"] = outputs[-1].replace(instruct_text, "")


        if top_p is not None:
            outputs = cls.generate_top_p(
                input_ids=instruct_ids_tensor,
                model=model,
                tokenizer=tokenizer,
                max_num_new_tokens=max_num_new_tokens,
            ...
            answer["top_p"] = outputs[-1].replace(instruct_text, "")


        if top_p is None and top_k is None:
            outputs = cls.generate_greedy(
                input_ids=instruct_ids_tensor,
                model=model,
                tokenizer=tokenizer,
                max_num_new_tokens=max_num_new_tokens,
            ...
            answer["greedy"] = outputs[-1].replace(instruct_text, "")
...
if __name__ == "__main__":
    ...
    input_ids_t = tokenizer_t.encode(text_t)
    input_ids_t = torch.tensor([input_ids_t]).to(device_t)

    config_t = Qwen3Config(vocab_size=tokenizer_t.vocab_size)
    model_t = Qwen3Model(config_t)
    state_dict = torch.load(BASE_MODEL_DIR/BASE_MODEL_NAME, map_location=device_t)
    model_t.load_state_dict(state_dict["model_state_dict"])
    model_t.to(device_t)

    print("[贪心采样]:", end="")

    result_t = Predictor.generate_greedy(
        input_ids=input_ids_t,
        model=model_t,
        tokenizer=tokenizer_t,
        max_num_new_tokens=256,
    ...
    print("[top-k采样]", end=":")
    result_t = Predictor.generate_top_k(
        input_ids=input_ids_t,
        model=model_t,
        tokenizer=tokenizer_t,
        max_num_new_tokens=256,
    ...
    print("[top-p采样]", end=":")
    result_t = Predictor.generate_top_p(
        input_ids=input_ids_t,
        model=model_t,
        tokenizer=tokenizer_t,
        max_num_new_tokens=256,
    ...
    #
    # question_t  = "太阳系有多少行星？"
    # answer_t = Predictor.generate_answer(
