...
from src.preprocess.dataset import GPTDataset, create_instruct_data_loader
...
class LoRATrainConfig:
    base_model_file: Path = BASE_MODEL_DIR/BASE_MODEL_NAME
    models_output_dir: Path = MODELS_DIR
    logs_dir: Path =  LOGS_DIR/"lora"
    ...
    lora_models_name = LORA_MODEL_NAME
...
class Trainer:
    ...
    def from_config(
            cls,
            model: Qwen3Model,
            train_cfg: LoRATrainConfig = train_config,
            train_dataset_file = str(INSTRUCT_PROCESSED_DIR/TRAIN_DATA_FILE),
            valid_dataset_file = str(INSTRUCT_PROCESSED_DIR/VALID_DATA_FILE),
    ...
    def train(
        ...
        if (self.config.lora_models_dir/self.config.lora_models_name).exists():
            try:
                tqdm.write(f"加载预训练模型...")
                checkpoint = torch.load(self.config.lora_models_dir/self.config.lora_models_name)
                if resume_model:
        ...
        self.model.train()
        # 准备数据加载器
        train_data_loader = create_instruct_data_loader(
        ...
        valid_data_loader = create_instruct_data_loader(
        ...
        entry = {"question": "中国的首都在哪里？", "answer": ""}
                ...
                if self.steps % self.config.eval_freq == 0:
                    ...
                    if valid_loss < self.min_valid_loss:
                        ...
                        torch.save({
                            ...
                            str(self.config.models_output_dir/self.config.lora_models_name)
                        )
...
if __name__ == "__main__":
    ...
    lora_model = create_lora_model(base_model, RANK, ALPHA)
